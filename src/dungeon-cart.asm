...
HP_ENERGY   = 26                ; Energy gained per HP used (+1 for movement)
FOOD_ENERGY = 26                ; Energy gained per food (+1 for movement)
...
CHR_KEY     = $0b               ; Key character
CHR_FILL    = $0d               ; Dungeon fill (all directions)
...
Welcome:    jsr $fd8d           ; Test RAM, initialize VIC chip
            ...
            sta HI_XP           ; ,,
            sta HI_XP+1         ; ,,
Restart:    jsr SetupHW         ; Set up hardware
...
; Level Up
LevelUp:    lda #" "            ; The key has been used, so remove it from the
            sta $1e62           ;   inventory display
...
InitLevel:  inc LEVEL           ; Increment player level counter
            ...
            ldy #21				; Color top and bottom of the maze
-loop:      lda #7				; ,,
            sta COLOR+132,y		; ,,
            sta COLOR+484,y 	; ,,
            dey					; ,,
...
DrumServ:   bit DRUMS_ON        ; Handle drum player
            bpl drum_r          ; ,,
            dec DRUMS_COUNT		; Each tick, decrement the count
            bmi drum_reset		; When the count elapses, start a new beat
            lda DRUMS_COUNT		; If we're halfway through the count,
            cmp #$03			;   then turn off the current percussion
            bne drum_r			;   tone. You know, so it seems percussive.
            lda #0				;   ,,
            sta VOICEL			;   ,,
            sta NOISE			;   ,,
            rts
drum_reset: lda #$06			; Set the tempo countdown
            sta DRUMS_COUNT		; ,,
            lda #0				; Rotate the 16-bit register left, bringing
            asl DRUMS_REG		;   bit 15 back to bit 0
            rol DRUMS_REG+1		;   ,,
            adc DRUMS_REG		;   ,,
            sta DRUMS_REG		;   ,,
play_drum:  bit DRUMS_REG		; Check a couple bits. If high bit is set,
            bpl snare			;   play a bass drum
            lda #$b0            ;   ,, (Bass Frequency)
            sta VOICEL			;   ,,
snare:      lda #$d0            ; Play snare or hats (Snare Frequency)
            bvc set_noise		;   depending on bit 6
...
Intro:      .asc $0d,$0d,$1e,"  2023 JASON JUSTIAN",$0d,$0d
...
CharSet:    ; Letters (* used,- reassignable)
            ...
            .byte $00,$00,$00,$60,$5e,$6a,$00,$00 ; Key 		   ($0b)
            .byte $40,$40,$a0,$a0,$40,$42,$7c,$00 ; L *
            ;.byte $82,$d6,$ba,$92,$aa,$92,$82,$00 ; M -
            .byte $ff,$ff,$ff,$ff,$ff,$ff,$ff,$ff ; Dungeon Fill   ($0d)
            ...
            ; Indicators
            .byte $b2,$ba,$92,$7c,$10,$1c,$24,$44 ; Dance Up       ($1b)
            .byte $18,$1c,$10,$1e,$10,$28,$24,$28 ; Dance Right    ($1c)
            .byte $00,$38,$38,$10,$7c,$92,$38,$c6 ; Dance Down     ($1d)
            .byte $18,$38,$08,$78,$08,$14,$24,$14 ; Dance Left     ($1e)
